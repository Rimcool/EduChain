...
class DegreeChecker
{
    ...
    public function check(array $data, ?int $userId = null): Verification
    {
        // In testing mode, return mock results based on test scenarios
        if (env('VERIFICATION_MODE') === 'testing') {
            return $this->mockCheck($data, $userId);
        }
    ...
    private function mockCheck(array $data, ?int $userId = null): Verification
    {
        // Mock scenarios for testing
        $studentName = strtolower(trim($data['student_name'] ?? ''));
        $universityName = strtolower(trim($data['university_name'] ?? ''));
        $degreeTitle = strtolower(trim($data['degree_title'] ?? ''));
        $rollNumber = strtolower(trim($data['roll_number'] ?? ''));
        $graduationYear = strtolower(trim($data['graduation_year'] ?? ''));

        // Scenario 1: Fully Verified Degree
        if ($studentName === 'ahmed ali khan' && 
            $universityName === 'comsats university, islamabad' &&
            $degreeTitle === 'bachelor of science in computer science' &&
            $rollNumber === 'fa19-bcs-001' &&
            $graduationYear === '2023') {
            ...
            return $this->save($data, $userId, [
                ...
                'layers' => [
                    ['pass' => true, 'name' => 'HEC Database', 'msg' => 'University confirmed: COMSATS University, Islamabad', 'grade' => 'A+'],
                    ['pass' => true, 'name' => 'Temporal Check', 'msg' => 'University operating since 1998, year 2023 valid', 'grade' => 'A+'],
                    ['pass' => true, 'name' => 'Degree Type', 'msg' => 'Degree type consistent with university category', 'grade' => 'A'],
                    ['pass' => true, 'name' => 'Blockchain', 'msg' => 'Hash match confirmed — university issued this degree', 'grade' => 'A+'],
                ],
            ], true, '✅ Fully Verified Degree');
        }

        // Scenario 2: Partially Verified (University not on EduChain)
        if ($studentName === 'sara noor' && 
            $universityName === 'university of karachi' &&
            $degreeTitle === 'bachelor of business administration' &&
            $rollNumber === '2019/bba/045' &&
            $graduationYear === '2022') {
            ...
            return $this->save($data, $userId, [
                ...
                'reason' => 'UNCONFIRMED: University is HEC accredited but has not joined EduChain. Cannot confirm or deny this specific degree.',
                'layers' => [
                    ['pass' => true, 'name' => 'HEC Database', 'msg' => 'University confirmed: University of Karachi', 'grade' => 'A+'],
                    ['pass' => true, 'name' => 'Temporal Check', 'msg' => 'University operating since 1951, year 2022 valid', 'grade' => 'A+'],
                    ['pass' => true, 'name' => 'Degree Type', 'msg' => 'Degree type consistent with university category', 'grade' => 'A'],
                    ['pass' => null, 'name' => 'Blockchain', 'msg' => 'University of Karachi has not joined EduChain yet', 'grade' => 'B'],
                ],
            ], true, '⚠️ Partially Verified (University not on EduChain)');
        }

        // Scenario 3: Fake University
        if ($universityName === 'oxford international college lahore') {
            return $this->save($data, $userId, [
                'result' => 'fake',
                'score'  => 0,
                'reason' => '"Oxford International College Lahore" is not recognized by HEC. This institution does not legally exist in Pakistan.',
                'layers' => [
                    ['pass' => false, 'name' => 'HEC Database', 'msg' => 'University not found in HEC database of 266 accredited institutions', 'grade' => 'F'],
                    ['pass' => null, 'name' => 'Temporal Check', 'msg' => 'Skipped', 'grade' => '—'],
                    ['pass' => null, 'name' => 'Degree Type', 'msg' => 'Skipped', 'grade' => '—'],
            ...
            ], true, '❌ Fake University');
        }

        // Scenario 4: Real University, Fake Degree
        if ($studentName === 'zara malik' && 
            $universityName === 'nust' &&
            $degreeTitle === 'bs software engineering' &&
            $rollNumber === 'nust-2019-fake' &&
            $graduationYear === '2023') {
            
            return $this->save($data, $userId, [
                'result' => 'fake',
                'score'  => 5,
                'reason' => 'FRAUDULENT: NUST is on EduChain but has no record of this degree.',
                'layers' => [
                    ['pass' => true, 'name' => 'HEC Database', 'msg' => 'University confirmed: NUST', 'grade' => 'A+'],
                    ['pass' => true, 'name' => 'Temporal Check', 'msg' => 'University operating since 1991, year 2023 valid', 'grade' => 'A+'],
                    ['pass' => true, 'name' => 'Degree Type', 'msg' => 'Degree type consistent with university category', 'grade' => 'A'],
                    ['pass' => false, 'name' => 'Blockchain', 'msg' => 'NUST is on EduChain but has NO record of this degree', 'grade' => 'F'],
                ],
            ], true, '❌ Real University, Fake Degree');
        }

        // Default mock result for unknown scenarios
        return $this->save($data, $userId, [
            'result' => 'unconfirmed',
            'score'  => 50,
            'reason' => 'UNCONFIRMED: Test scenario not recognized. This is a mock result.',
            'layers' => [
                ['pass' => true, 'name' => 'HEC Database', 'msg' => 'Mock university confirmed', 'grade' => 'A+'],
                ['pass' => true, 'name' => 'Temporal Check', 'msg' => 'Mock temporal check passed', 'grade' => 'A+'],
                ['pass' => true, 'name' => 'Degree Type', 'msg' => 'Mock degree type check passed', 'grade' => 'A'],
                ['pass' => null, 'name' => 'Blockchain', 'msg' => 'Mock blockchain check skipped', 'grade' => 'B'],
            ],
        ], true, 'Mock Test Scenario');
    }
    ...
    private function save(array $data, ?int $userId, array $outcome, bool $isTest = false, string $testScenario = ''): Verification
    {
